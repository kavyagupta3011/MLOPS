...
| MLflow Registry | a real registry — DB-backed tracking store, register + promote to "Production", tagged with which index/alpha config goes with the checkpoint | `MLFLOW_TRACKING_URI` (with the local MLflow server as fallback), `src/ci/promote_to_registry.py` (manual, named run), `src/ci/auto_promote_best.py` (automatic, picks the winner) |
